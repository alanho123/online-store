...
describe('OrderItem e2e test', () => {
  ...
  // const orderItemSample = {"quantity":10547,"totalPrice":23507.82,"status":"AVAILABLE"};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/products',
      body: {"name":"disorientate diffuse","description":"phew correctly","price":3735.66,"productSize":"M","image":"Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci5wbmc=","imageContentType":"unknown"},
    }).then(({ body }) => {
      product = body;
    });
    // create an instance at the required relationship entity:
    cy.authenticatedRequest({
      ...
      body: {"placedDate":"2024-07-22T23:34:39.283Z","status":"PENDING","code":"knock for"},
    }).then(({ body }) => {
      productOrder = body;
    });
  });
  ...
  describe('new OrderItem page', () => {
    ...
    it.skip('should create an instance of OrderItem', () => {
      cy.get(`[data-cy="quantity"]`).type('7091');
      cy.get(`[data-cy="quantity"]`).should('have.value', '7091');

      cy.get(`[data-cy="totalPrice"]`).type('4913.93');
      cy.get(`[data-cy="totalPrice"]`).should('have.value', '4913.93');
