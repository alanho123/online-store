...
describe('OrderItem e2e test', () => {
  ...
  // const orderItemSample = {"quantity":7655,"totalPrice":21490.94,"status":"OUT_OF_STOCK"};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/products',
      body: {"name":"bitter when immaculate","description":"gadzooks","price":25241.5,"productSize":"XL","image":"Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci5wbmc=","imageContentType":"unknown"},
    }).then(({ body }) => {
      product = body;
    });
    // create an instance at the required relationship entity:
    cy.authenticatedRequest({
      ...
      body: {"placedDate":"2024-07-23T00:22:02.315Z","status":"CANCELLED","code":"whoever if septicaemia"},
    }).then(({ body }) => {
      productOrder = body;
    });
  });
  ...
  describe('new OrderItem page', () => {
    ...
    it.skip('should create an instance of OrderItem', () => {
      cy.get(`[data-cy="quantity"]`).type('4130');
      cy.get(`[data-cy="quantity"]`).should('have.value', '4130');

      cy.get(`[data-cy="totalPrice"]`).type('13365.11');
      cy.get(`[data-cy="totalPrice"]`).should('have.value', '13365.11');
