...
describe('ProductCategory e2e test', () => {
  ...
  const productCategorySample = { name: 'supposing' };
  ...
  describe('new ProductCategory page', () => {
    ...
    it('should create an instance of ProductCategory', () => {
      cy.get(`[data-cy="name"]`).type('lightly train lest');
      cy.get(`[data-cy="name"]`).should('have.value', 'lightly train lest');

      cy.get(`[data-cy="description"]`).type('curvy feminise cloudy');
      cy.get(`[data-cy="description"]`).should('have.value', 'curvy feminise cloudy');
