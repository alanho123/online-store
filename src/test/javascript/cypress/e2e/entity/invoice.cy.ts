...
describe('Invoice e2e test', () => {
  ...
  // const invoiceSample = {"date":"2024-07-22T10:52:38.868Z","status":"PAID","paymentMethod":"PAYPAL","paymentDate":"2024-07-22T10:02:56.470Z","paymentAmount":14583.42};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      ...
      body: {"placedDate":"2024-07-22T18:36:31.707Z","status":"CANCELLED","code":"and arid"},
    }).then(({ body }) => {
      productOrder = body;
    });
  });
  ...
  describe('new Invoice page', () => {
    ...
    it.skip('should create an instance of Invoice', () => {
      cy.get(`[data-cy="date"]`).type('2024-07-22T16:23');
      cy.get(`[data-cy="date"]`).blur();
      cy.get(`[data-cy="date"]`).should('have.value', '2024-07-22T16:23');

      cy.get(`[data-cy="details"]`).type('legitimate');
      cy.get(`[data-cy="details"]`).should('have.value', 'legitimate');

      cy.get(`[data-cy="status"]`).select('CANCELLED');

      cy.get(`[data-cy="paymentMethod"]`).select('CASH_ON_DELIVERY');

      cy.get(`[data-cy="paymentDate"]`).type('2024-07-22T14:28');
      cy.get(`[data-cy="paymentDate"]`).blur();
      cy.get(`[data-cy="paymentDate"]`).should('have.value', '2024-07-22T14:28');

      cy.get(`[data-cy="paymentAmount"]`).type('10473.89');
      cy.get(`[data-cy="paymentAmount"]`).should('have.value', '10473.89');

      cy.get(`[data-cy="code"]`).type('nor rightfully aha');
      cy.get(`[data-cy="code"]`).should('have.value', 'nor rightfully aha');
