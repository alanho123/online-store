...
describe('Shipment e2e test', () => {
  ...
  // const shipmentSample = {"date":"2024-07-22T15:44:27.914Z"};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/invoices',
      body: {"date":"2024-07-22T04:00:45.460Z","details":"even gladly at","status":"ISSUED","paymentMethod":"CREDIT_CARD","paymentDate":"2024-07-22T19:36:08.885Z","paymentAmount":17646.13,"code":"vivaciously low curly"},
    }).then(({ body }) => {
      invoice = body;
    });
  });
  ...
  describe('new Shipment page', () => {
    ...
    it.skip('should create an instance of Shipment', () => {
      cy.get(`[data-cy="trackingCode"]`).type('jealously daintily');
      cy.get(`[data-cy="trackingCode"]`).should('have.value', 'jealously daintily');

      cy.get(`[data-cy="date"]`).type('2024-07-22T10:24');
      cy.get(`[data-cy="date"]`).blur();
      cy.get(`[data-cy="date"]`).should('have.value', '2024-07-22T10:24');

      cy.get(`[data-cy="details"]`).type('geez inside signify');
      cy.get(`[data-cy="details"]`).should('have.value', 'geez inside signify');
