...
public class ProductCategoryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all productCategories").get("/api/product-categories").headers(headers_http_authenticated).check(status().is(200))
            )
                ...
                .repeat(5)
                .on(
                    exec(http("Get created productCategory").get("${new_productCategory_url}").headers(headers_http_authenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created productCategory").delete("${new_productCategory_url}").headers(headers_http_authenticated))
                .pause(10)
        );
