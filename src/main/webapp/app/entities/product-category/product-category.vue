<template>
  <div>
    <h2 id="page-heading" data-cy="ProductCategoryHeading">
      <span v-text="t$('storeApp.productCategory.home.title')" id="product-category-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('storeApp.productCategory.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'ProductCategoryCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="t$('storeApp.productCategory.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && productCategories && productCategories.length === 0">
      <span v-text="t$('storeApp.productCategory.home.notFound')"></span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="productCategories">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="t$('storeApp.productCategory.name')"></span></th>
            <th scope="row"><span v-text="t$('storeApp.productCategory.description')"></span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span
          id="storeApp.productCategory.delete.question"
          data-cy="productCategoryDeleteDialogHeading"
          v-text="t$('entity.delete.title')"
        ></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-productCategory-heading" v-text="t$('storeApp.productCategory.delete.question', { id: removeId })"></p>
      </div>
