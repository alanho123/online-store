<template>
  <div>
    <h2 id="page-heading" data-cy="InvoiceHeading">
      <span v-text="t$('storeApp.invoice.home.title')" id="invoice-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('storeApp.invoice.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'InvoiceCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="t$('storeApp.invoice.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && invoices && invoices.length === 0">
      <span v-text="t$('storeApp.invoice.home.notFound')"></span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="invoices">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('date')">
              <span v-text="t$('storeApp.invoice.date')"></span>
            ...
            <th scope="row" v-on:click="changeOrder('paymentDate')">
              <span v-text="t$('storeApp.invoice.paymentDate')"></span>
            ...
            <th scope="row" v-on:click="changeOrder('paymentAmount')">
              <span v-text="t$('storeApp.invoice.paymentAmount')"></span>
            ...
            <th scope="row" v-on:click="changeOrder('code')">
              <span v-text="t$('storeApp.invoice.code')"></span>
            ...
            <th scope="row" v-on:click="changeOrder('order.code')">
              <span v-text="t$('storeApp.invoice.order')"></span>
        ...
        <tbody>
          <tr v-for="invoice in invoices" :key="invoice.id" data-cy="entityTable">
            ...
            <td v-text="t$('storeApp.InvoiceStatus.' + invoice.status)"></td>
            <td v-text="t$('storeApp.PaymentMethod.' + invoice.paymentMethod)"></td>
            <td>{{ formatDateShort(invoice.paymentDate) || '' }}</td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span id="storeApp.invoice.delete.question" data-cy="invoiceDeleteDialogHeading" v-text="t$('entity.delete.title')"></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-invoice-heading" v-text="t$('storeApp.invoice.delete.question', { id: removeId })"></p>
      </div>
