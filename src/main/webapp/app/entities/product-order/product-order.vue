<template>
  <div>
    <h2 id="page-heading" data-cy="ProductOrderHeading">
      <span v-text="t$('storeApp.productOrder.home.title')" id="product-order-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('storeApp.productOrder.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'ProductOrderCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="t$('storeApp.productOrder.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && productOrders && productOrders.length === 0">
      <span v-text="t$('storeApp.productOrder.home.notFound')"></span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="productOrders">
        ...
        <tbody>
          <tr v-for="productOrder in productOrders" :key="productOrder.id" data-cy="entityTable">
            ...
            <td v-text="t$('storeApp.OrderStatus.' + productOrder.status)"></td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span
          id="storeApp.productOrder.delete.question"
          data-cy="productOrderDeleteDialogHeading"
          v-text="t$('entity.delete.title')"
        ></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-productOrder-heading" v-text="t$('storeApp.productOrder.delete.question', { id: removeId })"></p>
      </div>
